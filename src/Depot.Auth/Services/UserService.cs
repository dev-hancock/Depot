...
public interface IUserService
{
    ...
    Task<ErrorOr<User>> RegisterAsync(string username, string password, IEnumerable<string> roles, CancellationToken token = default);
...
public class UserService : IUserService
{
    ...
    public async Task<ErrorOr<User>> RegisterAsync(string username, string password, IEnumerable<string> roles,
        CancellationToken token = default)
    {
        await using var context = await _factory.CreateDbContextAsync(token);

        var exists = await context.Users.AnyAsync(x => x.Username == username, token);

        if (exists)
        {
            return Error.Conflict("USER_EXISTS", "Username already exists.");
        }
        ...
        {
            var role = await context.Roles.SingleOrDefaultAsync(x => x.Name == name, token);
    ...
    public async Task<ErrorOr<User>> GetUserAsync(Guid id, CancellationToken token = default)
    {
        await using var context = await _factory.CreateDbContextAsync(token);
